...
/**
 * A modal confirmation for settings.
 *
 * Settings used to ask destructive questions with `window.confirm`, which the
 * browser renders as a chrome-owned "localhost says…" box — unstyled, untitled,
 * and unable to report why the action then failed. This dialog carries the same
 * shape as the chat and notes confirmations, and stays open on failure so the
 * server's reason ("you are the only admin") lands somewhere the user is
 * already looking.
 *
 * Focus is trapped and returned on close, and Escape cancels.
 *
 * Rendered through a portal onto `document.body`: settings rows live inside
...
export function ConfirmDialog({
  ...
  return createPortal(
    ...
    >
      ...
      >
        <h2 id="settings-confirm-title" className="text-lg font-bold text-fg-primary mb-2">
          {title}
        </h2>
        <p id="settings-confirm-message" className="text-sm text-fg-secondary mb-6">
          {message}
        </p>
